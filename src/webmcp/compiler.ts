    ...
    inputSchema: {
      ...
      properties: {
        ...
        content_html: {
          type: "string",
          description:
            "A complete preview document. component: include React/ReactDOM UMD scripts from unpkg.com and the Tailwind Play CDN; JSX may use @babel/standalone. You cannot import this app's own source files — if reproducing an existing component (this codebase's design system or one shown to you), write real JSX and Tailwind classes inline that recreate its actual markup, not a generic stand-in. static_html runs no JavaScript at all, but nothing else is restricted: a real web font (a Google Fonts <link>), full CSS (grid, gradients, custom properties, mix-blend-mode), and inline SVG filters (feTurbulence for film grain, feColorMatrix for duotone) are all available for posters, editorial layouts, and other high-fidelity visual work. Either renderer can use an existing image (a logo, a real photo) instead of a placeholder: drop its embed_url — from get_context_for_task or inspect_context_item — straight into an <img src>. The preview has no access to the host app, storage, forms, navigation, or arbitrary network requests beyond what's named here.",
        },
        renderer: {
          type: "string",
