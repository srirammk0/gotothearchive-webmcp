    ...
    <dl className="flex flex-col">
      {rows
        ...
        .map((r) => (
          ...
          >
            <dt className="shrink-0 text-[length:var(--text-micro)] text-faint">{r.label}</dt>
            <dd className="min-w-0 truncate text-right text-[length:var(--text-micro)] text-muted">{r.value}</dd>
          </div>
        ))}
    </dl>
  );
