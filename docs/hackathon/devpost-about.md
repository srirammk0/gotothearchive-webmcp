...
The agent works from what you allowed and returns its result to the Workbench as a version you can review. You see what it looked at, what actually shaped the output, and who made it. You annotate it, approve it, send it back, or ask for another pass. If it built a component, it runs live in the Workbench so you can click through it.

From your feedback, the agent can propose a taste signal tied to the exact notes behind it. Nothing slips into a profile on its own. You edit it, scope it, reject it, or keep it.
...
Retrieval was a decision point. The easy path was a managed search service, but that hides the permission model in a box I don't control, and its indexing lags, which breaks the whole "flip a lock and the next call fails" promise. So retrieval runs directly on the database. It filters by permission first, then ranks. Something you can't see is gone from the results, not sitting there with a low score.

The part I got most wrong was taste. I built it server-side first. It grouped your notes by topic and wrote the preference sentence from a template, and the output was things like "Leans away from the current typography on posters." Vague, and exactly the background guessing this project exists to stop. So I deleted it. Now a taste signal exists only because an agent read your feedback and named the pattern with evidence, or because you wrote it yourself. The server's job there is to keep that evidence honest when you edit a note later, nothing more.

Partway through I ran the whole thing through an outside WebMCP audit and fixed what it flagged, mostly about when tools register and how their descriptions read.

## Challenges

Making permissions real in every place at once. The interface, the agent's tools, retrieval, and the server all had to agree, including in the second right after you revoke something.
...
Revocation going deep enough. A taste signal is a compressed version of the material behind it. If a folder taught a preference, revoking that folder has to pull the preference too, not just the folder's items.

## What I learned

WebMCP can be more than a cleaner way for agents to operate a site. It can make the page itself the shared control surface between you and an agent, where the thing that organizes your work is the same thing that sets the boundary.

A schema is good for stating authority but it cannot enforce it. That has to happen at runtime, on every call. And provenance matters as much as retrieval. People need to know what shaped a result, not just what the agent opened.
